class Invoice < ApplicationRecord
  ...
  def discounted_revenue
    ...
    # best_bulk_discounts = invoice_items.select("invoice_items.id, MAX(bulk_discount.percentage_discount / 100) * invoice_items.unit_price * invoice_items.quantity as total_discount")
    #               .joins(item: {merchant: :bulk_discounts})
    #               .where("bulk_discounts.threshold_quantity <= invoice_items.quantity")
    #               .group("invoice_items.id")


    # best_bulk_discounts.sum { |invoice_item| invoice_item.total_discount}
  ...
  def total_discount
    discounted_revenue.sum(&:total_discount)
  end
